...
// ▼▼▼ 修正: isEmbedded（埋め込みフラグ）を受け取る ▼▼▼
export default function Community({ isEmbedded = false }) {
  const items = [
    {
      title: "社長ブログ",
  ...
  return (
    // 埋め込み時は背景色や最小高さを無効化
    <div className={isEmbedded ? "" : "bg-slate-50 min-h-screen"}>
      
      {/* ▼▼▼ 修正: 埋め込み時はヘッダー（Hero）を表示しない ▼▼▼ */}
      {!isEmbedded && (
      ...
      <Section
        ...
        // ▼▼▼ 修正: 埋め込み時は上下の余白を調整して詰める ▼▼▼
        className={isEmbedded ? "py-10 md:py-12" : "pt-8"}
      >
      ...
      {/* ▼▼▼ 修正: 埋め込み時は下部のCTA（お問い合わせエリア）を表示しない ▼▼▼ */}
      {!isEmbedded && (
